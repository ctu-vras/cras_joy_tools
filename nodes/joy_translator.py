...
def find_config_for_device(device, base_path):
    ...
    information, it returns the config file $VENDOR:$PRODUCT.yaml (if it exists) or other yaml file if it contains
    this device ID in its property `device_ids`.
    ...
    if os.path.exists(devices_file):
        ...
        with open(devices_file, 'r') as devices:
            ...
            device_id = "%s:%s" % device_vendor_product
...
class JoyTranslator(object):
    ...
    def translate(self, joy):
        ...
        if len(joy.buttons) != self.config['num_buttons']:
            rospy.logerr(
                "The received message has %i buttons, but the config requires %i of them" % (
        ...
        while not rospy.is_shutdown():
            ...
            if os.path.exists(device):
                if translator is None:
                    ...
                    config_file = find_config_for_device(device, config_base_path)
